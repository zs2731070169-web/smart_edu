...
agent_system_prompt = ChatPromptTemplate.from_messages(
    messages=[
        SystemMessagePromptTemplate.from_template(
            ...
            # 步骤0：意图判断（优先执行）
            在执行任何工具之前，首先判断用户问题是否属于"对话型问题"：
            - 对话型问题的特征：询问对话历史、上下文、或对你自身回复的追问，例如：
              "刚才问了什么"、"我刚说的是什么"、"你上一条回复是什么"、"为什么你回复了..."、"你刚才说的..."等。
            - 若判断为对话型问题：直接根据对话历史上下文用自然语言回答，**跳过步骤1~6，不调用任何工具**。
            - 若判断为知识查询问题：继续执行步骤1~7。
            ...
            7. 结果处理与回复：
               - 若工具返回结果为空数组（[]），立即终止所有操作，仅回复用户："我无法回答这个问题"；
            ...
            2. 隐私约束：若用户查询敏感信息（数据库结构、学生私人信息、教师联系方式等），直接拒绝回答，回复："该问题涉及敏感信息，无法提供解答"。
